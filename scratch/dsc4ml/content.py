...
for γ in [0, 1]:
    ...
    plt.scatter(*enc.protos.T, c=np.sign(enc.protos @ w), marker='x', s=100)
    ...
    losses['total'].plot(style='o--', ax=plt.subplot(1, 3, 1), title=f'Total loss ($\gamma = {γ:.2f}$)')
    losses['mse'].plot(style='o--', ax=plt.subplot(1, 3, 2), title='MSE loss')
    losses['zero_one'].plot(style='o--', ax=plt.subplot(1, 3, 3), title='0-1 loss')
    plt.show()
...
for γ in [0, 1]:
    ...
    for i in range(30):
        dec = dec.optimize(X, enc, γ, w)
        enc = enc.optimize(X, dec, calc_total_loss(γ))
        losses.append({'Train': calc_total_loss(γ)(X, dec(enc(X))).mean().item(),
                       'Test': calc_total_loss(γ)(Z, dec(enc(Z))).mean().item()})
